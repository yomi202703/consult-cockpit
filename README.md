...
- `server.py` — SSE ハブ / タブ制御スレッド(CDP は1本に直列化)/ consult ループ / ルート
- `gemma_chat.py` — free-form streaming クライアント(env_loader → OpenAI 互換 /chat/completions, stream=True)
- `static/index.html` — 3レーン UI(EventSource + fetch POST、依存なし)
- `run.sh` — PYTHONPATH を張って improver venv で server.py を exec
